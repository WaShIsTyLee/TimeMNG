...
public class UpdateChildController extends Controller implements Initializable {
    @FXML
    ...
    public Child takeValuesChild() {
        ...
        childaux = new Child(childName, childSurname, childClass, childObservation, diseasesList, childAge,childID, minder, tdao.findByMail(childEmailTeacher));
    ...
    @FXML
    public void updateChildToBD() throws IOException {
        Child child = takeValuesChild();
        ChildDAO cdao = new ChildDAO();
        TeacherDAO tdao = new TeacherDAO();
        Teacher teacher = tdao.verifyTeacherEmailIfExist(child.getTeacher().getEmail());
        if (teacher != null && child.getId() > 0 &&  cdao.findById(child.getId()) != null) {
            cdao.update(child);
        } else {
